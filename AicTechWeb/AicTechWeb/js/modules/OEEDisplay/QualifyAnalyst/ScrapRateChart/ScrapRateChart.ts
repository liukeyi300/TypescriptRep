...
module AicTech.Web.Html {
    export class ScrapRateChart extends OEEChartBase<ScrapRtDataModel> {
        private allParValueList;
        private allScrpDataForShow = {
            shiftData: null,
            dateData: null,
            weekData: null,
            monthData: null
        ...
        constructor() {
            ...
            $.extend(this.viewModel, kendo.observable({
                ...
                advanceData: function (e) {
                    (<ScrapRateChart>Module.ModuleLoad.getModuleInstance('ScrapRateChart')).redrawChart(RedrawStatu.Advance);
                },
                backoffData: function (e) {
                    (<ScrapRateChart>Module.ModuleLoad.getModuleInstance('ScrapRateChart')).redrawChart(RedrawStatu.Backoff);
                }
            }));
        ...
        private getAllData(start: Date, end: Date, equId: string, callback: Function) {
            var instance = <ScrapRateChart>Module.ModuleLoad.getModuleInstance("ScrapRateChart"),
                currentData: ScrapRtDataModel,
                recString;
            instance.serviceContext.V_PPA_QA_RECORD
                ...
                .toArray((re) => {
                    ...
                    if (re.length <= 0) {
                        ...
                        re.forEach((it) => {
                            currentData = new ScrapRtDataModel(it);
         ...
         * 计算各个周期的数据
         */
        protected pretreatData(allData: ScrapRtDataModel[]) {
            var instance = <ScrapRateChart>Module.ModuleLoad.getModuleInstance('ScrapRateChart'),
                currentParValue,
                i,
                length,
            ...
            allData.forEach((it: ScrapRtDataModel) => {
                ...
                if (instance.allWeek.length === 0) {
                    ...
                    var lastDay: Date = instance.allScrpDataForShow.weekData[instance.allWeek[instance.allWeek.length - 1]].recTime,
        ...
        refreshData() {
            ...
            try {
                (function (instance: ScrapRateChart) {
